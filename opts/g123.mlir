"builtin.module"() ({
  "pdl.pattern"() ({
    ...
    %5 = "pdl.operation"(%1, %4, %0) {attributeValueNames = [], opName = "arith.divsi", operand_segment_sizes = array<i32: 2, 0, 1>} : (!pdl.value, !pdl.value, !pdl.type) -> !pdl.operation
    "pdl.rewrite"(%5) ({
      "pdl.replace"(%5, %1) {operand_segment_sizes = array<i32: 1, 0, 1>} : (!pdl.operation, !pdl.value) -> ()
    }) {operand_segment_sizes = array<i32: 1, 0>} : (!pdl.operation) -> ()
  }) {benefit = 1 : i16, sym_name = "opt123"} : () -> ()
}) : () -> ()
